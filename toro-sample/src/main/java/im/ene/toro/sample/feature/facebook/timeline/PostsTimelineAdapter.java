...
public class PostsTimelineAdapter extends ToroAdapter<ToroAdapter.ViewHolder>
    ...
    public PostsTimelineAdapter(ArrayList<TimelineItem> items, AlbadiyaTimelineFragment fragment,PostsTimlineFragment timeline) {
        this.delegate = new MediaPlayerManagerImpl();
        this.items = items;
    ...
    private PostsTimelineAdapter.ItemClickListener onItemClickListener;

    public void setOnItemClickListener(PostsTimelineAdapter.ItemClickListener onItemClickListener) {
        this.onItemClickListener = onItemClickListener;
    }

    @Override public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        final ViewHolder viewHolder = PostsTimelineViewHolder.createViewHolder(parent, viewType,fragment,timeline);
        ...
        } else if (viewHolder instanceof PostsVideoViewHolder) {
            // TODO Click to Video
            viewHolder.setOnItemClickListener(new View.OnClickListener() {
                @Override public void onClick(View v) {
                    int position = viewHolder.getAdapterPosition();
                    if (position != RecyclerView.NO_POSITION
                            && onItemClickListener != null
                            && v == ((PostsVideoViewHolder) viewHolder).getPlayerView()) {
                        onItemClickListener.onVideoClick(viewHolder, v,
                                (TimelineItem.VideoItem) getItem(position).getEmbedItem());
                    }
                }
            });
        } else if (viewHolder instanceof PostsPhotoViewHolder) {
            // TODO Click to Photo
        }
